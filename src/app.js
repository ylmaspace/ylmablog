...
window.YLMACommunityBridge = {
  getProfile() {
    const me = state.me || {};
    return {
      name: me.name || "GuestPilot",
      xp: me.XP ?? 0,
      nova: me.NOVA ?? 0,
      avatar: "🧑‍🚀",
      role: me.role || "user",
  ...
  applyReward({ xp = 0, nova = 0, reason = "foro" } = {}) {
    if (!state.me) return;
    state.me.XP = (state.me.XP || 0) + xp;
    state.me.NOVA = (state.me.NOVA || 0) + nova;
    logEvent(`FORO reward: +${xp} XP / +${nova} NOVA (${reason})`);
    render();
  },
};
